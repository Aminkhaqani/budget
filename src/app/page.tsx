              ...
              <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
                {(['month', 'season', 'year'] as const).map(p => (
                  <Button
                    ...
                    size="sm"
                    className={`flex-1 rounded text-xs h-8 ${period === p ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : ''}`}
                    onClick={() => setPeriod(p)}
                  >
              ...
              <div className="space-y-2">
                ...
                <Card className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white border-0 shadow-md">
                  <CardContent className="p-4">
                    ...
                    <p className="text-emerald-100 text-xs mt-2">{accounts.length} accounts</p>
                  </CardContent>
                </Card>
                      ...
                      {previousIncome > 0 && (
                        <div className="flex items-center gap-1 mt-1 text-xs">
                          ...
                          <span className={incomeChange >= 0 ? 'text-emerald-500' : 'text-red-500'}>
                            {Math.abs(incomeChange).toFixed(0)}% vs prev
                          </span>
                        </div>
                      )}
                    </CardContent>
                      ...
                      {previousExpenses > 0 && (
                        <div className="flex items-center gap-1 mt-1 text-xs">
                          ...
                          <span className={expenseChange <= 0 ? 'text-emerald-500' : 'text-red-500'}>
                            {Math.abs(expenseChange).toFixed(0)}% vs prev
                          </span>
                        </div>
                      )}
                    </CardContent>
              ...
              <Card className="border shadow-sm">
                <CardContent className="p-3">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
                      <Button
                        variant="ghost"
                        size="sm"
                        className={`h-7 px-3 text-xs rounded ${chartType === 'expense' ? 'bg-white dark:bg-slate-700 shadow-sm' : ''}`}
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        className={`h-7 px-3 text-xs rounded ${chartType === 'income' ? 'bg-white dark:bg-slate-700 shadow-sm' : ''}`}
                    ...
                    {chartData.length > 0 && (
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <div className="text-center">
                          ...
                          <p className="text-base font-bold">{formatCurrency(chartTotal)}</p>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 justify-center">
                    ...
                    {chartData.length > 5 && (
                      <span className="text-xs text-slate-400">+{chartData.length - 5} more</span>
                    )}
                  </div>
                </CardContent>
              </Card>
